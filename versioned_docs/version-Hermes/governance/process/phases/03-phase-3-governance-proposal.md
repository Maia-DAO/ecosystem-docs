...
2. Create your proposal. This can be done either through an interface (on **[[Governance Interface](https://app.maiadao.io/governance/create-proposal)]**) or through writing the calldata for more complicated proposal logic. This calldata will be executed if and when the proposal passes. If writing the calldata yourself, please review the logic with a qualified Hermes community member prior to posting the proposal. 
...
Congratulations! You've made it to the end of all three phases of the Hermes Governance Process. Ok, so now what? Now we await the votes to finalize and we can gauge whether or not the proposal will pass while meeting quorum, or not pass. 
